...
!(function () {
  ...
  (function(){
	  window.loading={
		createL:function(){
			...
			this.layout.className='loading';
			this.layout.style.display='none';
			this.layout.innerHTML+='<div class="loadBox"><img src="../images/loading.gif"/><span>loading…</span></div>';
		},
		appendL:function(){
			var loadParent=document.querySelector('.content');
		    loadParent.appendChild(loading.layout);
		    loading.load=document.querySelector('.loading');
		    loading.load.style.display='block';
		},
		open:function(){
			if(loading.load){
				loading.load.style.display='block';
		...
		close:function(){
			if(loading.load){
				loading.load.style.display='none'
			}else{return}
		}
	  }
	  loading.createL();
  ...
  var success = function (xhr, options) {
    ...
    swapContent(data.contents, options.container, options.transition, function () {
      ...
      window.myJs=document.getElementsByTagName('h1')[0];
      switch(myJs.title){
      case 'password':
         // keyReset();
    	  setPassWord({
              ...
              fn:function(passWord){
            	  ...
            	  if(passWord=="error")
            	{
            		...
            		PUSH({
            			url : document.getElementById("appName").value+'/yqrx/toBangKaInit.do?tradePass=' + passWord
            		});
            	}
              }
            });
